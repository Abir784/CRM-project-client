...
switch ($action) {
    ...
    case 'json_list':
        ...
        foreach ($x as $xs) {
            $records["data"][] = [
                ...
                '
                <div class="btn-group">
                <a href="#" class="btn btn-success btn-xs emview" id="emid' .
                $xs['id'] .
    ...
    case 'email':
        ...
        $msg = '';
        $cid = _post('lid');
        $d = ORM::for_table('crm_leads')->find_one($cid);
        $email = $d['email'];
        $toname = $d['salutation']." ".$d['first_name']." ".$d['middle_name']." ".$d['last_name'];
    ...
    case 'groups':
        ...
        $gs = ORM::for_table('leads_groups')
            ->order_by_asc('sorder')
            ->find_array();

        $ui->assign('gs', $gs);

        $ui->assign('xfooter', Asset::js(['leads/groups']));
 ...
 '
    ...
    case 'add_group':
        ...
        if ($group_name != '') {
            ...
            $d = ORM::for_table('leads_groups')->create();
            $d->gname = $group_name;
            $d->color = '';
            $d->discount = '';
            $d->parent = '';
    ...
    case 'find_by_group':
        ...
        if ($gid) {
            $g = ORM::for_table('leads_groups')->find_one($gid);
...
'
    ...
    case 'group_edit':
        ...
        $d = ORM::for_table('leads_groups')->find_one($id);
